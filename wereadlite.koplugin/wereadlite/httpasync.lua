...
local function read_response_body(sock, resp_headers)
    local body = {}
...
-- @treturn boolean ok
-- @return on success: content_type, body, resp_headers
-- @return on failure: err, content_type or nil, body or nil, resp_headers or nil
function HttpAsync.fetch_url(url, redirect_count, headers)
    ...
    local ua = socketutil.USER_AGENT
    if headers then
        ua = headers["User-Agent"] or headers["user-agent"] or ua
    end
    table.insert(req_lines, string.format("User-Agent: %s", ua))
    if headers then
        for k, v in pairs(headers) do
            if type(k) == "string" and k:lower() ~= "user-agent" and k:lower() ~= "host" then
    ...
    local content = read_response_body(sock, resp_headers)
    sock:close()
    local content_type = resp_headers["content-type"]

    if code and code >= 400 then
        return false, "HTTP error " .. tostring(code), content_type, content, resp_headers
    end

    return true, content_type, content, resp_headers
end
...
function HttpAsync.fetch_many(tasks, opts)
    ...
    local fetch = opts.fetch or function(url)
        local ok, a, b, c = HttpAsync.fetch_url(url)
        if ok then
            return true, b
        end
        return false, c or a
